...
router.get("/nuevo", preventCache, ensureAuthenticated, async (req, res) => {
  try {
      let {valor} = await divisasController.consultarValor();
      const consulta = await ventasController.crearVenta();      
      const { codigo, clientes, productos, fecha_creacion } = consulta;      
      res.render('ventas/crearVenta', {codigo: codigo,clientes: clientes, productos: productos, fecha_creacion: fecha_creacion, valorDivisa: valor, session: req.session})      
    } catch (error) {
      res.render("error", { message: "Error al cargar el formulario de creación de Ventas" + error });
    }
});

router.post("/guardar", preventCache, ensureAuthenticated, upload.none(), async (req, res) =>{
  try { 
    const datosVenta = req.body
    const rol = req.session.user.roles
    const nuevaVenta = await ventasController.registrarVenta(datosVenta);
    res.status(200).json({success: true});   
  } catch (error) {
    console.log('Error en la ruta /guardar:', error.message);
