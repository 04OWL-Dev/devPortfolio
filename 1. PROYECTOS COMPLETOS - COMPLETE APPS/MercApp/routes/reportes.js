...
router.get("/notaEntrega/:id", async (req, res) => {
    ...
    try {
        ...
        const logoPath = './public/img/logoHD.png';
        ...
        const totalTextWidth = doc.widthOfString(`Total: ${datosVenta.encabezado.total_venta.toFixed(2)}$`);
        const tasaWidth = doc.widthOfString(`Tasa Referencial BCV: ${datosVenta.encabezado.precio_divisa} Bs`);
        let xPosition = 60 + 220 + 90 + 90 + 5 + totalTextWidth - tasaWidth;
        doc.text(`Tasa Referencial BCV: ${datosVenta.encabezado.precio_divisa} Bs`, xPosition, doc.y + (20 - 15));
        ...
        xPosition = 60 + 220 + 90 + 90 + 5 
        doc.fontSize(12).text(`Total: ${datosVenta.encabezado.total_venta.toFixed(2)}$`, xPosition, doc.y + (rowHeightTable));       

        if (datosVenta.encabezado.descuento > 0) {
            // Definir los textos y sus respectivos valores
            const texts = [
                { label: `Descuento: ${datosVenta.encabezado.descuento.toFixed(2)}%`, value: datosVenta.encabezado.total_descuento },
                { label: `Subtotal($): ${datosVenta.encabezado.total_descuento.toFixed(2)}$`, value: datosVenta.encabezado.total_descuento },
                { label: `Subtotal(Bs): ${datosVenta.encabezado.total_descuento_bs.toFixed(2)}Bs`, value: datosVenta.encabezado.total_descuento_bs },
            ];        
            // Calcular la posición en X para que los textos terminen en el mismo margen derecho que "Total"
            const totalTextWidth = doc.widthOfString(`Total: ${datosVenta.encabezado.total_venta.toFixed(2)}$`);            
            texts.forEach((item, index) => {
                const textWidth = doc.widthOfString(item.label);
                const xPosition = 60 + 220 + 90 + 90 + 5 + totalTextWidth - textWidth;        
                // Dibuja el texto alineado al margen derecho de "Total"
                doc.text(item.label, xPosition, doc.y + (rowHeightTable - 15));
            });
        }
...
router.post('/reporteCompras', async (req, res) => {
    try {
        ...
        // Devuelve la URL directa al PDF generado
        res.status(200).json({ redirectUrl: `/reportes/reportePDF?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&tipo=${tipo}` });
    } catch (err) {
        console.error('Error al generar el reporte de compras:', err);
...
router.get('/reportePDF', async (req, res) => {
    try {
        ...
        // Función para dibujar tabla con salto de página automático
        function drawTable(doc, startX, startY, rowHeight, rows) {
            ...
            rows.forEach((item, index) => {
                ...
                if (currentY + rowHeight > doc.page.height - doc.page.margins.bottom) {
                    ...
                    currentY = doc.page.margins.top; // Restablecer la posición Y
                }
